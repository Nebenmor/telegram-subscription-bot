...
class AdminHandler {
  ...
  async handleSetupMessage(msg, groupId = null) {
    ...
    try {
      ...
      if (!groupId) {
        const adminGroups = database.getGroupsByAdmin(userId);
        const groupInSetup = adminGroups?.find(group => 
  ...
  async startSetupProcess(query, group, groupId) {
    ...
    await this.bot.editMessageText(
      MESSAGES.SETUP_START(groupName),
      {
        chat_id: query?.message?.chat?.id,
        message_id: query?.message?.message_id,
        parse_mode: 'Markdown'
  ...
  async handleEditConfigCallback(query) {
    ...
    try {
      ...
      await this.bot.editMessageText(
        MESSAGES.EDIT_CONFIG_START(groupName),
        {
          chat_id: query?.message?.chat?.id,
          message_id: query?.message?.message_id,
          parse_mode: 'Markdown',
  ...
  async handleConfirmEditCallback(query) {
    ...
    try {
      ...
      await this.bot.editMessageText(
        MESSAGES.EDIT_CONFIG_CONFIRMED,
        {
          chat_id: query?.message?.chat?.id,
          message_id: query?.message?.message_id,
          parse_mode: 'Markdown'
  ...
  async showCurrentConfig(userId, group) {
    ...
    try {
      ...
      const groupId = group.groupId || Object.keys(database.data.groups || {}).find(
  ...
  async handlePaymentNotification(userId, userDetails, receiptFileId = null, receiptType = null) {
    ...
    try {
      ...
      if (!group) {
        console.error(`Group ${userDetails.groupId} not found for payment notification`);
        return;
      }
...
Please add the user to the group manually, then click the button below to confirm.`;
      ...
      if (receiptFileId && receiptType) {
        await this.forwardReceipt(userId, receiptFileId, receiptType, userDetails);
      } else {
        await this.bot.sendMessage(userId, 
  ...
  async forwardReceipt(adminId, receiptFileId, receiptType, userDetails) {
    try {
      ...
      console.log(`📄 Receipt forwarded to admin ${adminId} from user ${userDetails.username}`);
    } catch (error) {
  ...
  async handleUserAddedCallback(query) {
    ...
    if (!match) {
      return this.bot.answerCallbackQuery(query?.id, { text: 'Invalid callback data' });
    }
    ...
    try {
      ...
      await this.bot.answerCallbackQuery(query.id, { text: 'User added successfully' });

      console.log(`✅ User ${username} added to group "${groupName}" (${groupId}) with ${subscriptionDays} access`);
  ...
  async handleRefreshGroups(query) {
    ...
    try {
      ...
      if (!adminGroups?.length) {
        await this.bot.editMessageText(
          MESSAGES.NO_GROUPS_FOUND,
          {
            chat_id: query?.message?.chat?.id,
            message_id: query?.message?.message_id
          }
        );
        return this.bot.answerCallbackQuery(query.id, { text: 'No groups found' });
      }
